...
namespace Framework.Shared.Helpers
{
    ...
    public class ServiceExecutionHelper
    {
        ...
        public T GetDataAsync<T>(Func<T> codeBlock, string cacheKey = null, int cacheExpirationTimeInSecs = 0, int asyncAfterSecs = 0, string apiURL = "", object postedData = null, bool IsManualKey = false)
        {
            ...
            var cacheMetaInfo = GenerateCacheKey(list, frame, IsManualKey);

            cacheKey = string.IsNullOrEmpty(cacheKey) ? cacheMetaInfo.CacheKey : string.Format("CK_{0}_{1}__{2}", cacheMetaInfo.ClassName, cacheMetaInfo.MethodName, cacheKey);
        ...
        private CacheKeyAndMasterKey GenerateCacheKey(Delegate[] list, StackFrame frame, bool IsManualKey)
        {
            ...
            if (list != null & list.Length > 0)
            {
                ...
                cacheKey = string.Format("CK_{0}_{1}", className, methodName);
                int counter = 0;
