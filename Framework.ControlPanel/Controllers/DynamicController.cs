...
namespace Framework.ControlPanel.Controllers
{
    public class DynamicController : BaseController
    {
            ...
            bool? allowImport, string customJavaScript, string dateFilters, string customQuery, bool? showAllGridColumns, bool? allowApprovals)
        {
            ...
            foreach (var field in model.PageInfo.Fields)
            {
                ...
                if (field.IsForeignkey) //This will be used for search filled ups
                {
                    ...
                    var rFeilds = refrencedTable.Fields.Where(x => x.IsPrimaryKey || x.Name.Contains("Name") || x.Name.Contains("Value") || x.Name.Contains("Number") || x.Name.Contains("Description") || x.Name.Contains("Code") || x.Name.Contains("OrganizationID")).Select(x => x.Name).ToList();
                    ...
                    //else
                    //{
                    field.RefrencedTableData = CommonServices.Instance.GetTableListingData(field.RefrencedTableName, rFeilds, null, string.Empty, null, 1, 100, false, null, OrgID: Authentication.Instance.User.OrganizationID);
                    //}
                }
        ...
        {
            string columName = string.Format("{0}NameAr", tableName.Singlofy());
        ...
        public ActionResult AddEditTableRecord(string tableName, object id, string excludeAddEditColumns, string extenderName, string secondaryTableConfigs, bool? dontLoadRecursiveData, bool? allowApprovals, short? tableEntityID)
        {
            ...
            foreach (var field in model.PageInfo.Fields)
            {
                if (field.IsForeignkey)
                {
                    ...
                    //else
                    //{
                    field.RefrencedTableData = CommonServices.Instance.GetTableListingData(field.RefrencedTableName, rFeilds, null, string.Empty, null, 1, 100, false, string.Empty, OrgID: Authentication.Instance.User.OrganizationID);
                    //}
                }
        ...
        public JsonResult SaveUpdateTableRecord(FormCollection form)
        {
            ...
            foreach (string key in form.Keys)
            {
                ...
                if (key != "TableName" || key != "IsNew" || key != "ExtenderName" || key != "AllowApprovals" && key != "TableEntityID")
                {
                    ...
                    else if (key == "OrganizationID" && string.IsNullOrEmpty(form[key]))
                    {
                        record.Add(key, Authentication.Instance.User.OrganizationID.ToString());
                    }
                    else if (key == "OrganizationName" && string.IsNullOrEmpty(form[key]))
                    {
                        record.Add(key, Authentication.Instance.User.OrganizationName);
                    }
                    else if (key == "PictureURL")
                    {
                        record.Add(key, Authentication.Instance.User.PictureURL);
                    }
                    else
                    {
            ...
            if (tableName == "Roles" || tableName == "Users" || tableName == "Fields" || tableName == "RationAmount")
            {
                Framework.Shared.DataServices.CustomEntities.DynamicField a = new Framework.Shared.DataServices.CustomEntities.DynamicField();
                a=fields.ElementAt(0);
                //a.Name = "OrganizationID";
                //a.DisplayNameAr = "OrganizationID";
                //a.DbTypeID = 52;
                //a.IsRequired = false;
                //a.IsForeignkey = true;
                //a.IsAutoID = true;
                ...
                fields.Add(a);
                //record.Add("OrganizationID", Authentication.Instance.User.OrganizationID);
        ...
        {
            var obj = Activator.CreateInstance("Framework.ControlPanel", string.Format("Framework.ControlPanel.CommonCode.{0}", name));
